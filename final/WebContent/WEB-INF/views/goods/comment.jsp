	...
	if (comment.commentShape == '0') {
		...
		+ '<div class="panel-heading">'
			...
			+ '<button type="button" class="btn btn-info btn-xs" onclick="commentReply('+comment.commentGroupNo+', '+comment.commentSeq+', \''+comment.commentShape+'\')">답변</button> '
				...
				+ ' <button type="button" class="btn btn-danger btn-xs" onclick="location.href=\'../comment/delete.do?seq='+comment.commentSeq+'&page=${param.page}&boardUrl=good&bbsSeq=${param.seq}\'">삭제</button>'
			+ '</div>'
	+ '</div>'
...
function commentUpdate(i) {	
	...
	$("#comment_"+ i).prepend(
			'<input type="hidden" name="boardUrl" value="good">'
